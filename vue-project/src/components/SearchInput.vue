  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== "") {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=0e2b0600d5c049ed9ec143745240106&q=
${searchTerm.query}`
      );
...
const getWeather = async (id) => {
  const res = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=0e2b0600d5c049ed9ec143745240106&q=id:${id}&days=3&aqi=no&alerts=no`
  );
  const data = await res.json();
